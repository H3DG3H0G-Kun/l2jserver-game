...
public abstract class DocumentBase {
	...
	protected Condition parseLogicAnd(Node n, Object template) {
		...
		if ((cond.conditions == null) || (cond.conditions.length == 0)) {
			LOG.error("Empty <and> condition in file {}!", _file);
		}
		return cond;
	}
	...
	protected Condition parseLogicOr(Node n, Object template) {
		...
		if ((cond.conditions == null) || (cond.conditions.length == 0)) {
			LOG.error("Empty <or> condition in file {}!", _file);
		}
		return cond;
	}
	...
	protected Condition parseLogicNot(Node n, Object template) {
		...
		LOG.error("Empty <not> condition in file {}!", _file);
		return null;
	}
	...
	protected Condition parsePlayerCondition(Node n, Object template) {
		...
		for (int i = 0; i < attrs.getLength(); i++) {
			...
			switch (a.getNodeName().toLowerCase()) {
				...
				default -> LOG.error("Unrecognized <player> condition {} in file {}!", a.getNodeName().toLowerCase(), _file);
			}
		}
		
		if (cond == null) {
			LOG.error("Unrecognized <player> condition in file {}!", _file);
		}
		return cond;
	}
	...
	protected Condition parseTargetCondition(Node n, Object template) {
		...
		if (cond == null) {
			LOG.error("Unrecognized <target> condition in file {}!", _file);
		}
		return cond;
	}
	...
	protected Condition parseUsingCondition(Node n) {
			...
			switch (a.getNodeName().toLowerCase()) {
				case "kind" -> {
					...
					while (st.hasMoreTokens()) {
						...
						if (old == mask) {
							LOG.warn("[parseUsingCondition=\"kind\"] Unknown item type name: {} in string {}!", item, a.getNodeValue());
						}
					}
				...
				case "slot" -> {
					...
					while (st.hasMoreTokens()) {
						...
						if (old == mask) {
							LOG.warn("[parseUsingCondition=\"slot\"] Unknown item slot name {}!", item);
						}
					}
		...
		if (cond == null) {
			LOG.error("Unrecognized <using> condition in file {}!", _file);
		}
		return cond;
	}
	...
	protected Condition parseGameCondition(Node n) {
		...
		if (cond == null) {
			LOG.error("Unrecognized <game> condition in file {}!", _file);
		}
		return cond;
	}
