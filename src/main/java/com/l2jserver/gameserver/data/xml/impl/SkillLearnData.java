...
public final class SkillLearnData implements IXmlReader {
	...
	public void parseDocument(Document doc) {
		for (var node = doc.getFirstChild(); node != null; node = node.getNextSibling()) {
			if ("list".equalsIgnoreCase(node.getNodeName())) {
				for (var list_node = node.getFirstChild(); list_node != null; list_node = list_node.getNextSibling()) {
					if ("npc".equalsIgnoreCase(list_node.getNodeName())) {
						final var classIds = new LinkedList<ClassId>();
						for (var c = list_node.getFirstChild(); c != null; c = c.getNextSibling()) {
